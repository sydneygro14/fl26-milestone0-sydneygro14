...
The const version gives read-only access; the non-const version allows editing. The compiler picks whichever one fits based on whether the Workspace itself is const.
